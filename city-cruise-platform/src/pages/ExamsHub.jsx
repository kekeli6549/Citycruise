...
import { Award, Lock, ShieldCheck, Timer, BookOpen } from 'lucide-react';
              ...
              >
                <div className="bg-white dark:bg-brand-dark rounded-[1.9rem] md:rounded-[2.4rem] p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
                  ...
                  <button 
                    ...
                    className={`w-full md:w-auto px-8 py-4 rounded-xl font-bold uppercase text-[10px] tracking-widest transition-all ${isUnlocked ? 'bg-slate-900 dark:bg-white text-white dark:text-brand-dark hover:bg-brand-blue hover:text-white' : 'bg-slate-50 dark:bg-slate-900/50 text-slate-400 cursor-not-allowed'}`}
                  >
                    {isUnlocked ? 'Start Assessment' : 'Locked'}
                  </button>
                </div>
              </motion.div>
            );
